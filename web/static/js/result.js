...
function renderDetailMetrics(m, data = {}) {
  ...
  const rows = [
    ...
    ['成本前年化收益', Fmt.pct(m.gross_annual_return ?? m.annual_return), Fmt.colorClass(m.gross_annual_return ?? m.annual_return)],
    ['成本后年化收益', Fmt.pct(m.net_annual_return ?? m.annual_return), Fmt.colorClass(m.net_annual_return ?? m.annual_return)],
    ['交易成本拖累', Fmt.pct(m.cost_drag || 0, 2), (m.cost_drag || 0) > 0.03 ? 'text-yellow-400' : ''],
    ['平均日换手率', Fmt.pct(m.avg_daily_turnover || 0, 2), ''],
    ['年化换手率', Fmt.pct(m.annual_turnover || 0, 1), (m.annual_turnover || 0) > 12 ? 'text-yellow-400' : ''],
