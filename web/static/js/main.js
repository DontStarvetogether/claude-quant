...
async function loadUniversePresets() {
  try {
    const data = await API.getUniverses();
    SYMBOL_PRESETS = (data.universes || []).map(u => ({
      id: u.id,
      label: u.name,
      source: u.source,
