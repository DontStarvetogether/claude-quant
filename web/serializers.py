...
def serialize_result(record: RunRecord) -> BacktestResultResponse:
    ...
    metrics = MetricsDict(
        ...
        avg_daily_turnover=_safe_float(getattr(m, "avg_daily_turnover", 0.0)) or 0.0,
        annual_turnover=_safe_float(getattr(m, "annual_turnover", 0.0)) or 0.0,
        max_daily_turnover=_safe_float(getattr(m, "max_daily_turnover", 0.0)) or 0.0,
        buy_turnover=_safe_float(getattr(m, "buy_turnover", 0.0)) or 0.0,
        sell_turnover=_safe_float(getattr(m, "sell_turnover", 0.0)) or 0.0,
        total_turnover=_safe_float(getattr(m, "total_turnover", 0.0)) or 0.0,
        gross_return=_safe_float(getattr(m, "gross_return", m.total_return)) or 0.0,
        net_return=_safe_float(getattr(m, "net_return", m.total_return)) or 0.0,
        gross_annual_return=_safe_float(getattr(m, "gross_annual_return", m.annual_return)) or 0.0,
        net_annual_return=_safe_float(getattr(m, "net_annual_return", m.annual_return)) or 0.0,
        total_slippage_cost=_safe_float(getattr(m, "total_slippage_cost", 0.0)) or 0.0,
        cost_drag=_safe_float(getattr(m, "cost_drag", 0.0)) or 0.0,
        cost_to_nav=_safe_float(getattr(m, "cost_to_nav", 0.0)) or 0.0,
        avg_position_count=_safe_float(getattr(m, "avg_position_count", 0.0)) or 0.0,
        max_position_count=int(getattr(m, "max_position_count", 0) or 0),
        min_position_count=int(getattr(m, "min_position_count", 0) or 0),
        avg_cash_ratio=_safe_float(getattr(m, "avg_cash_ratio", 0.0)) or 0.0,
        average_exposure=_safe_float(getattr(m, "average_exposure", 0.0)) or 0.0,
        max_single_position_weight=_safe_float(getattr(m, "max_single_position_weight", 0.0)) or 0.0,
        avg_top5_concentration=_safe_float(getattr(m, "avg_top5_concentration", 0.0)) or 0.0,
    ...
    for t in result.trades:
        ...
        trades.append(
            TradeRecord(
                ...
                fee=round(t.commission + t.stamp_tax, 2),
                slippage_adjusted_price=round(t.price, 4),
            )
        )
    ...
    return BacktestResultResponse(
        ...
        execution_assumptions=result_state.get("execution_assumptions"),
        metric_diagnostics=result_state.get("metric_diagnostics"),
